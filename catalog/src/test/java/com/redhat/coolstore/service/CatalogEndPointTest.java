...
public class CatalogEndPointTest {
	...
	/*  Hoverfly is a unit test library for testing service to service communication.
	 *  Simulate the existence of non-live service.
	 * 
	 */
	@ClassRule
...
//	                    .willReturn(serverError())
	                    		    .willReturn(success(json(new Inventory("329299",0))))
	                    ));
	
	@Test
	public void test_retriving_one_proudct() {
		
		ResponseEntity<Product> response = restTemplate.getForEntity("/service/product/329299", Product.class);
...
//TODO: Add check for Quantity
	...
	@Test
	public void test_that_endpoint_returns_a_correct_list() {
		
		ResponseEntity<List<Product>> rateResponse = 
				restTemplate.exchange("/services/products", HttpMethod.GET, null, new ParameterizedTypeReference<List<Product>>() {
                });	
